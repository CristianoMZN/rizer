// ─── Mock API (legado) ───────────────────────────────────────────────────────
// Mantido por compatibilidade com páginas/componentes que ainda dependem
// de `api.getVehicles()`, `api.getStores()`, `api.getLeads()` etc.
// Use `tenantApi`/`adminApi` em `src/services/api.ts` para chamadas reais.
...
const delay = (ms = MOCK_CONFIG.delay) => new Promise<void>((r) => setTimeout(r, ms))

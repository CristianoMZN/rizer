...
async function load() {
  ...
  if (!MOCK_CONFIG.useBackend) {
    ...
    if (!store) {
      error.value = 'Parceiro não encontrado'
      loading.value = false
      return
    }
    ...
    products.value = MOCK_VEHICLES
      ...
      .map((v): PublicProductView => ({
        ...
        yearModel: v.year, yearBuild: v.year, mileageKm: v.mileage,
        fuel: v.fuel, transmission: v.transmission,
        brandName: v.brand, modelName: v.model,
        categoryName: v.subtype ?? v.type,
